...
namespace aspect
{
  namespace Particle
  {
    namespace Property
    {
       ...
       * olivine aggregate with an ad hoc dehydration boundary
       *
       * @ingroup ParticleProperties
       */
      template <int dim>
      class GrainOr70 : public Interface<dim>, public ::aspect::SimulatorAccess<dim>
      {
        public:
          /**
	 ...
	 * fractions, and the per-grain strain energies, in an aggregate
        ...
        double max_dep; // maximum model depth
      };
    }
  }
}
...
namespace aspect
{
  namespace Particle
  {
    namespace Property
    {
      template <int dim>
      void
      GrainOr70<dim>::initialize_one_particle_property(const Point<dim> &,
	...
	for (unsigned int i=0; i<n_grains; ++i)
      ...
      template <int dim>
      void
      GrainOr70<dim>::update_one_particle_property(const unsigned int data_position,
                                                const Point<dim> &position,
                                                const Vector<double> &,
                                                const std::vector<Tensor<1,dim> > &gradients,
                                                const ArrayView<double> &data) const
	...
	unsigned int n_grains = data[data_position];  // retrieve n_grains for parsing the rest
       ...
       tau_dry[2] = 3;
       tau_dry[3] = 1e60;
       for (unsigned int i=0; i<4; ++i) tau_pos[i] = tau[i];
       if (dim==2)
         {
	...
	// step 1
	std::vector<double> dot_acs_odf_rt = calculate_derivs(lij,eij,eps0,n_grains,tau_pos,acs_odf_rt);  // calculate derivatives/rt
	...
	odf_test = 0;
	odf_sum = 0;
	dot_acs_odf_rt = calculate_derivs(lij,eij,eps0,n_grains,tau_pos,acs_odf_rt_i);  // derivatives/rt from intermediate
	...
	odf_test = 0;
	odf_sum = 0;
	dot_acs_odf_rt = calculate_derivs(lij,eij,eps0,n_grains,tau_pos,acs_odf_rt_i);
	...
	for (unsigned int i=0; i<n_grains; ++i)
	  ...
	  for (unsigned int j=0; j<4; ++j) qab[j] = std::abs(bigI[j]/tau_pos[j]);
	  ...
	  double rat = tau_pos[imax]/bigI[imax];
	  double qint = rat*bigI[iint]/tau_pos[iint];
	  double qmin = rat*bigI[imin]/tau_pos[imin];
	  ...
	  double rt1 = std::pow(tau_pos[imax],1.5-stress_exp)*std::pow(std::abs(gam[imax]*gam0),1.5/stress_exp);
	  double rt2 = std::pow(tau_pos[iint],1.5-stress_exp)*std::pow(std::abs(gam[iint]*gam0),1.5/stress_exp);
	  double rt3 = std::pow(tau_pos[imin],1.5-stress_exp)*std::pow(std::abs(gam[imin]*gam0),1.5/stress_exp);
	  double rt4 = std::pow(tau_pos[inac],1.5-stress_exp)*std::pow(std::abs(gam[inac]*gam0),1.5/stress_exp);
	...
	return dot_all;
      ...
      template <int dim>
      UpdateTimeFlags
      GrainOr70<dim>::need_update() const
      {
      ...
      template <int dim>
      UpdateFlags
      GrainOr70<dim>::get_needed_update_flags () const
      {
      ...
      template <int dim>
      std::vector<std::pair<std::string, unsigned int> >
      GrainOr70<dim>::get_property_information() const
      {
	...
	return property_information;
      ...
      template <int dim>
      void
      GrainOr70<dim>::declare_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
	 {
          prm.enter_subsection("Particles");
	   {
	     prm.enter_subsection("Grain orientations");
	     {
	       prm.declare_entry("Number of grains","500",Patterns::Integer(1),
              ...
              prm.declare_entry("Maximum model depth","3e5",Patterns::Double(0),
					"Maximum model depth in meters");
            }
      ...
      template <int dim>
      void
      GrainOr70<dim>::parse_parameters (ParameterHandler &prm)
      {
	 prm.enter_subsection("Postprocess");
	 {
	   prm.enter_subsection("Particles");
	   {
	     prm.enter_subsection("Grain orientations");
	     {
              ...
              max_dep = prm.get_double("Maximum model depth");
            }
            prm.leave_subsection();
          }
          prm.leave_subsection();
...
namespace aspect
{
  namespace Particle
  {
    namespace Property
    {
      ASPECT_REGISTER_PARTICLE_PROPERTY(GrainOr70,
                                        "grain orientations",
